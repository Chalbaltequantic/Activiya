...
public function fetchSupplier(Request $request)
{
    try {
        ...
        if (empty($supplier_code)) {
            ...
            return response()->json([
                'status' => false,
                'message' => 'Supplier code missing'
            ]);
        }

        $supplier = Siteplant::where('plant_site_code', $supplier_code)
            ->first();
        ...
        return response()->json([
            'status' => true,
            'supplier_name' => $supplier->plant_site_location_name ?? '',
            'supplier_location' => $supplier->city ?? '',
        ]);
...
public function fetchConsignee(Request $request)
{
    try {

        $consignee_code = trim((string)$request->consignee_code);

        if (empty($consignee_code)) {

            return response()->json([
                'status' => false,
                'message' => 'Consignee code missing'
            ]);
        }

        $consignee = Siteplant::where('plant_site_code', $consignee_code)
            ->first();
        ...
        return response()->json([
            'status' => true,
            'consignee_name' => $consignee->plant_name ?? '',
            'consignee_location' => $consignee->location ?? '',
        ]);
...
public function fetchMaterial(Request $request)
{
    try {
        ...
        $m_code = trim((string)$request->m_code);

        if (empty($m_code)) {

            return response()->json([
                'status' => false,
                'message' => 'Material code missing'
            ]);
        }

        $material = MaterialMaster::where('m_code', $m_code)
            ->first();
...
public function fetchTransporter(Request $request)
{
    try {

        $transporter_code = trim((string)$request->transporter_code);

        if (empty($transporter_code)) {

            return response()->json([
                'status' => false,
                'message' => 'Transporter code missing'
            ]);
        }

        $transporter = Vendor::where('vendor_code', $transporter_code)
            ->first();
	...
	public function fetchTruck(Request $request)
	{
		try {
			...
			if (empty($truck_code)) {
				...
				return response()->json([
					'status' => false,
					'message' => 'Truck code missing'
				]);
			}

			$truck = TruckMaster::where('code', $truck_code)
				->first();
