...
class MappingController extends Controller
{
    ...
    {
        $title = 'Mapping Data Upload';
        $pagetitle = $title.' Listing';
       
		$data = $request->all();        
	    $mappingdatalist = Mapping::orderBy('created_at', 'desc')->get();       
        return view('admin.mapping.index',compact(['pagetitle','title','mappingdatalist']));
    }
	//mappingdatalist
	public function mappingdatalist(Request $request)
    {
        $title = 'Mapping Data ';
        $pagetitle = $title.' Listing';
       $user_role = Auth::user()->role_id;
		$data = $request->all();        
	    $mappingdatalist = Mapping::orderBy('created_at', 'desc')->get();       
        return view('admin.mapping.mappingdatalist',compact(['pagetitle','title','mappingdatalist','user_role']));
    }
	public function getMappingdataDetails($id)
	{
		$mappingdata = Mapping::find($id);
		$userid = auth()->user()->id; //get loggedin user id		
		return view('admin.mapping.editmappingdata', compact('mappingdata'));
	}
	...
	public function save_mappingdata(Request $request)
	{
		$validatedData = $request->validate(
			...
			return Redirect('/admin/mapping/mappind-data-list')->with('success', 'Mapping data updated successfully!');
		
	}
	
	
	public function DeleteMappingData($id)
	{
		//$mappingdata = Mapping::find($id);		
		Mapping::find($id)->delete();
		return Redirect('/admin/mapping/mappind-data-list')->with('success', 'Mapping data deleted successfully!');
	}
	...
	{
		$userid = auth()->user()->id; //get loggedin user id		
		return view('admin.mapping.manualupload');
	}
	...
	public function save_manual_mappingdata(Request $request)
	{
        ...
        try {
            for ($i = 0; $i < $count; $i++) {
              ...
              /*  $exists = Mapping::where('company_code', $data['company_code'])
            ...
            DB::commit();
           return Redirect('/admin/mapping/mappind-data-list')->with('success', 'Data imported successfully!');
