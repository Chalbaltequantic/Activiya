...
class LoadOptimizerService
{
    public function generate(string $referenceNo, bool $isEdit = false)
	{
		DB::transaction(function () use ($referenceNo, $isEdit) {
			...
			$origin_name = $skus->first()->origin_name;
			$destination_name = $skus->first()->destination_city;
			...
			LoadSummary::where('reference_no', 'LIKE', $referenceNo.'%')->delete();

			$remaining = $skus->values();
			$child = 0;

			$parentWeightCeiling = null;
			...
			while ($remaining->isNotEmpty()) {
				...
				/** 6️ FIT SKUs (apply parent ceiling BEFORE create) */
				$fit = collect();
				$w = 0;
				$v = 0;

				$weightLimit = $parentWeightCeiling ?? $truck->weight_capacity;

				foreach ($remaining as $sku) {
					if (
						($w + $sku->z_total_weight) <= $weightLimit &&
				...
				/** 7️ FREEZE PARENT CONSTRAINTS */
				if ($child === 0) {
					$parentWeightCeiling = $w;
					$maxTruckCapacity = $truck->weight_capacity;
				}

				/** 8️ CREATE SUMMARY (SAFE NOW) */
				...
				$summary = LoadSummary::create([
					...
					'approval_status' => $is_qualified ? 'APPROVED' : 'PENDING',
					'status' => 'ACTIVE'
				]);
   ...
   /* private function bestTruck($origin, $destination, $weight)
		...
		$maxCapacity = null)
	{
		return DB::table('truck_master as tm')
			...
			// Do not allow truck escalation after parent
			->when($maxCapacity, function ($q) use ($maxCapacity) {
				$q->where('tm.weight_capacity', '<=', $maxCapacity);
			})

			// Prefer trucks that can carry required weight
			->orderByRaw("
				CASE
					WHEN tm.weight_capacity >= ? THEN 0
					ELSE 1
				END
			", [$requiredWeight])

			// Best-fit: smallest adequate truck first
			->orderBy('tm.weight_capacity')
