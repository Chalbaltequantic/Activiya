								...
								<table class="table table-bordered border-dark table-hover" id="table"  style="table-layout:auto; width:auto;">
								  ...
								  <tbody>
									...
									<tr class="{{ in_array($appointdata->id, $mismatchedIds) ? 'table-danger mismatch-row' : '' }}" data-entry-id="{{ $appointdata->id }}"  data-id="{{ $appointdata->id }}">
										...
										@else
											<input type="file"
												id="pod_front_{{ $appointdata->id }}"
												class="pod-upload"
												data-type="podfront"
												data-id="{{ $appointdata->id }}"
												data-appointment-id="{{ $appointdata->id }}"
												data-file-type="podfront">

											<button type="button"
												class="btn btn-sm btn-primary mt-2"
												onclick="capturePhoto('pod_front_{{ $appointdata->id }}')" id="cameraFrontBtn">
												📷
											</button>
										@endif</td>
										...
										@else
											<input type="file"
												id="pod_back_{{ $appointdata->id }}"
												class="pod-upload"
												data-type="podback"
												data-id="{{ $appointdata->id }}"
												data-appointment-id="{{ $appointdata->id }}"
												data-file-type="podback">

											<button type="button"
												class="btn btn-sm btn-primary mt-2"
												onclick="capturePhoto('pod_back_{{ $appointdata->id }}')" id="cameraBackBtn">
												📷
											</button>
										@endif
...
$(document).ready(function () {
    ...
    $('.pod-upload').on('change', function () {
        ...
        $.ajax({
            ...
            beforeSend: function () {
                fileInput.after('<em class="uploading text-primary">Uploading...</em>');
            },
...
<!-- WORKING CAMERA CAPTURE SCRIPT -->
<script>
function capturePhoto(originalInputId) {
    ...
    cameraInput.onchange = function (e) {
        ...
        // Attach captured file to original real input
