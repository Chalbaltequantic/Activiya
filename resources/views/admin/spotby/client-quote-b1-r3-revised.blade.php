...
<style>
 ...
 .sticky-col-4 {
      position: sticky;
      left: 235px; /* Adjust based on col-1 width */
      background: #fff;
      z-index: 99;
    }
            ...
            <div class="card">
              <div class="card-header p-2">
                <ul class="nav nav-pills">
                  <li class="nav-item"><a class="nav-link active" href="#activity" data-toggle="tab">User B1 _Round 3 (Buyer)</a></li>
                  <li class="nav-item"><a class="nav-link" href="#timeline" data-toggle="tab">User B1 _Round 3 (Buyer) Quote</a></li>
						  ...
						  @foreach($historyQuotes as $historyspotbydata)
							...
							@endphp
							   ...
							   <tr data-spotby-id="{{ $historyspotbydata->id }}">
								...
								<td>
								{{$historyspotbydata->freeze_vendor_name}}		
								</td>
								<td>
								{{$historyspotbydata->final_rate}}		
								</td>
								</tr>
						  @endforeach
						  @endif
...
$('#clientOfferForm').on('submit', function(e) {
    e.preventDefault();
	var $btn = $("#saveAllVendors"); // your save button
    ...
    $.ajax({
        url: "{{ route('admin.client.offers.storer3') }}",
        type: "POST",
        data: $(this).serialize(),
