...
<!-- Content Header (Page header) -->
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Indent Allocation</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
             <li class="breadcrumb-item"><a href="/admin/dashboard">Dashboard</a></li>
             <li class="breadcrumb-item active">Indent Allocation</li>
            ...
            <div class="card">
              <div class="card-header p-2">
                <ul class="nav nav-pills">
				  ...
				  <li class="nav-item"><a class="nav-link active" href="{{ route('admin.loadsummary_auto_allocation') }}">Indent Allocation</a></li>
				   <li class="nav-item"><a class="nav-link" href="{{ route('admin.track.placement.status') }}">Track Placement Status</a></li>
                  	...
                  	<div class="table-responsive-fixed border rounded shadow-sm bg-white consign-data-table table-container">
						<table id="appointdataTable" class="table table-bordered border-dark table-hover">
							...
							<tr>
								...
								<th style="background: #fce4d6; color: #0070c0;z-index:999;" class="sticky-col-3">Destination<br>name code</th>
								...
								<td>{{ $row->t_mode }}</td>
									...
									<td>
										...
										<button
											...
											data-id="{{ $row->id }}" data-source="{{ $row->source_type }}">
											Edit
										</button>
									...
									<td>
										...
										onclick="sendToVendor({{ $row->id }},'{{ $row->vendor_code }}',{{ $row->vendor_rank }},'{{ $row->vendor_code_source }}','{{ $row->reference_no }}',	'{{ $row->origin_name_code }}',	'{{ $row->destination_name_code }}','{{ $row->source_type }}' )">Send</button>	
...
$(document).on('click', '.editVendorBtn', function () {
    ...
    let id     = $(this).data('id');
    let source = $(this).data('source'); // AUTO / MANUAL

    $.get("{{ url('/admin/loadsummary/edit-vendor') }}", {
        id: id,
        source: source
    }, function (res) {

        $('#load_summary_id').val(res.load.id);
        $('#allocation_source').val(source); // hidden input
        ...
        res.vendors.forEach(v => {
            options += `
                <option value="${v.vendor_code}Rank${v.rank}" data-name="${v.vendor_name}">
...
$('#saveVendor').click(function () {
    ...
    $.post("{{ route('admin.loadsummary.update.vendor') }}", {
        ...
        load_summary_id: $('#load_summary_id').val(),
		 allocation_source: $('#allocation_source').val(),
...
function sendToVendor(loadId, vendor_code, vendor_rank, vendor_code_source, refno, origin, destination,    source_type ) 
{
    Swal.fire({
        ...
        html: `
            ...
            <textarea id="sendRemarks" class="swal2-textarea" placeholder="Enter remarks (required)"></textarea>`,
        showCancelButton: true,
        confirmButtonText: 'Send',
        preConfirm: () => {
            const remarks = $('#sendRemarks').val();
            if (!remarks) {
        ...
        $.post("{{ route('admin.vendor.send') }}", {
            ...
            source_type: source_type,          // AUTO / MANUAL
            vendor_code_source: vendor_code_source,  // Auto Allocation / Manual Edit
